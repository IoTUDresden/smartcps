 ...
 * add sections and headers. Used for display device types, rooms and first
 * letter of name in {@linkplain DashboardActivity}
 * 
 * @author Tonic Artos, edit by Andreas Hippler
 */
public class DashboardStickyGridHeadersAdapterWrapper extends BaseAdapter implements StickyGridHeadersBaseAdapter {
	private DashboardListAdapter mDelegate;
	 ...
	 */
	public DashboardStickyGridHeadersAdapterWrapper(DashboardListAdapter adapter) {
		mDelegate = adapter;
	...
	public int getCountForHeader(int position) {
		return mDelegate.isSorted() ? mHeaders[position].getCount() : mDelegate.getCount();
	}
	...
	public int getNumHeaders() {
		return mDelegate.isSorted() ? mHeaders.length : 0;
	}
