...
/**
 * Time continuous bar chart. Every x seconds a new bar is added, old bars are
 * shifted left.
 * 
 * @author Tobias Hafermalz <s5900703@mail.zih.tu-dresden.de>
 */
public class BarChart extends ContinuousVisualization {
	...
	private void addBar() {
		...
		MarginLayoutParams marginParams = new MarginLayoutParams(mBar.getLayoutParams());
		marginParams.setMargins(0, 0, oldLayoutParams.width / 2, 0);
		...
		int textMargin = (oldLayoutParams.width + oldLayoutParams.width / 2);
	...
	private void showIdeal() {
		...
		mIdeal.setBegin(Double.valueOf(from * 100).floatValue());
		mIdeal.setEnd(Double.valueOf(to * 100).floatValue());
